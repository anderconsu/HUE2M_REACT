...
const FormIngredients = () => {
    ...
    try {
      ...
      if (ingEnglish.status === 200) {
        ...
        ingEnglish = {
          ...
          cantidad: parseInt(e.target.cantidad.value),
          unidad:
            e.target.unidad.value === "units" ? "" : e.target.unidad.value,
        };
      } else {
  ...
  const getEdamamData = async () => {
    ...
    ingListEnglish.forEach((ing) => {
      ingredients.push(`${ing.cantidad} ${ing.unidad} ${ing.ingrediente}`);
    });
        ...
        <section className="noDbUser">
          {error ? (
            <article className="incompleteProfile">
              <p>{error} </p> <Link to="/profile">Completar perfil</Link>
            </article>
          ) : null}
        </section>
      );
      ...
      return (
        <section className="IngredientList">
          <article className="text">
            ...
            <p>
              Te recomendamos, hacerlo una vez ya hayas completado todas las
              comidas.
            </p>
          </article>
          ...
          {edamamError === "Cargando..." ? <p>Cargando...</p> : null}

          {edamamError === "555" ? (
            <p>
              Hay un error al cotejar uno o varios ingredientes. Puede que
              alguno de los ingredientes no sea cotejable, por favor,
              compruebalo eliminando los ingredientes
            </p>
          ) : null}
          ...
          <section className="buttons">
            <button
              ...
              disabled={blockSubmit === true || ingListEnglish.length === 0}
            >
              Calcular
            </button>
            <button
              className="deleteList"
              disabled={blockSubmit === true || ingList.length === 0}
              onClick={() => {
                setIngList([]);
                setIngListEnglish([]);
              }}
